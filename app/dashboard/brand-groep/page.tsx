...
export default function GroupToBrandAvailablePage() {
  ...
  const drilldownAvailableTotal = useMemo(() => {
    return productRows.reduce((sum, r) => sum + (r.available || 0), 0)
  }, [productRows])

  // reset product selection als merk/groep wisselt
  useEffect(() => {
    setSelectedProduct(null)
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Selecteer een <span className="font-medium">groep 1</span>, klik op een{' '}
              <span className="font-medium">merk</span>, en klik op een product voor details. (som{' '}
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {selectedGroupMeta && (
                        <div className="text-xs text-gray-500">
                          Totaal beschikbaar:{' '}
                          <span className="font-semibold">{formatInt(selectedGroupMeta.availableTotal)}</span> •{' '}
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-700">
                    <input
                      type="checkbox"
                      checked={top10Brands}
                      onChange={e => setTop10Brands(e.target.checked)}
                      disabled={!selectedGroup}
                    />
                    ...
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={4} className="px-6 py-10 text-center text-gray-500">
                          Kies links een groep en klik rechts op een merk om producten te zien.
                        </td>
                      </tr>
